...
/*
 * POST request to append auth token
 */
async function fetchPOSTRequest (
  ...
  try {
    ...
    if (resp.ok) return response
      ...
      const errorResp = { statusCode: resp.status }
      const errors = {
        error: new Error(),
        ...response,
        ...errorResp,
      }
      console.info(`${method} API FAILED, ${url}`, errors)
      // STATUS CODE 401 is for unauthorised user
      if (
        resp.status === 401
      ) {
